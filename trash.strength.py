...
for path,obj,lists in os.walk("trash_img"):
    for i in lists:
        # 随机旋转
        rotate = rotation(open_img('trash_img/'+i))
        rotate.save('trash_save/' + i[:-4] + '_' + str(1) + '.png')
        # 随机平移
        trans = shift(open_img('trash_img/'+i), random.randint(0, 5000), random.randint(0, 5000))
        trans.save('trash_save/' + i[:-4] + '_' + str(2) + '.png')
        # 随机修剪
        crp = crop(open_img('trash_img/'+i))
        crp.save('trash_save/' + i[:-4] + '_' + str(3) + '.png')
        # 左右翻转
        flip_h = flip(open_img('trash_img/'+i), 0)
        flip_h.save('trash_save/' + i[:-4] + '_' + str(4) + '.png')
        # 上下翻转
        flip_v = flip(open_img('trash_img/'+i), 1)
        flip_v.save('trash_save/' + i[:-4] + '_' + str(5) + '.png')
        # 对比度增强
        contrast = contrast_enhancement(open_img('trash_img/'+i))
        contrast.save('trash_save/' + i[:-4] + '_' + str(6) + '.png')
        # 亮度增强
        bright = brightness_enhancement(open_img('trash_img/'+i))
        bright.save('trash_save/' + i[:-4] + '_' + str(7) + '.png')
        # 色彩抖动
        color = random_color(open_img('trash_img/'+i))
        color.save('trash_save/' + i[:-4] + '_' + str(8) + '.png')
        # 噪声扰动
        noise = gaussian(open_img('trash_img/'+i))
        noise.save('trash_save/' + i[:-4] + '_' + str(9) + '.png')