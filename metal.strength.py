...
for path,obj,lists in os.walk("metal_img"):
    for i in lists:
        # 随机旋转
        rotate = rotation(open_img('metal_img/'+i))
        rotate.save('metal_save/' + i[:-4] + '_' + str(1) + '.png')
        # 随机平移
        trans = shift(open_img('metal_img/'+i), random.randint(0, 5000), random.randint(0, 5000))
        trans.save('metal_save/' + i[:-4] + '_' + str(2) + '.png')
        # 随机修剪
        crp = crop(open_img('metal_img/'+i))
        crp.save('metal_save/' + i[:-4] + '_' + str(3) + '.png')
        # 左右翻转
        flip_h = flip(open_img('metal_img/'+i), 0)
        flip_h.save('metal_save/' + i[:-4] + '_' + str(4) + '.png')
        # 上下翻转
        flip_v = flip(open_img('metal_img/'+i), 1)
        flip_v.save('metal_save/' + i[:-4] + '_' + str(5) + '.png')
        # 对比度增强
        contrast = contrast_enhancement(open_img('metal_img/'+i))
        contrast.save('metal_save/' + i[:-4] + '_' + str(6) + '.png')
        # 亮度增强
        bright = brightness_enhancement(open_img('metal_img/'+i))
        bright.save('metal_save/' + i[:-4] + '_' + str(7) + '.png')
        # 色彩抖动
        color = random_color(open_img('metal_img/'+i))
        color.save('metal_save/' + i[:-4] + '_' + str(8) + '.png')
        # 噪声扰动
        noise = gaussian(open_img('metal_img/'+i))
        noise.save('metal_save/' + i[:-4] + '_' + str(9) + '.png')